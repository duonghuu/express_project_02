...
export const responsysController = {
    async handleRegister(req: Request, res: Response): Promise<void> {
        ...
        //     endpoint
        // }
        const job = await jobQueue.add('sendRequest', { endpoint, name: req.body.name, email: req.body.email });
        fs.appendFileSync(
            "logs/receive.log",
        ...
        // await jobQueue.add("sendEmail", { to: `user${new Date().toISOString()}@example.com` });
        res.json({ success: true });
    },
};
